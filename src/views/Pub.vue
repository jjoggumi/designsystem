  ...
  <div class="post-export-wrap">
            ...
            <div class="option-list type01" v-if="modals.fetchGroups" v-click-outside="() => closeModal('fetchGroups')">
              <ul>
                ...
                <li v-else v-for="group in groups" :key="group.value">
                  <HiButton color="link" class="item" :class="{ 'is-selected': group.value === selectedGroup }" @click="openModal('reFetch')">
                    {{ group.title }}
                  </HiButton>
                  <div class="append">
    ...
    <HiModal type="type01" size="sm" v-if="modals.groupEdit" closeSkip @close="closeModal('groupEdit')">
      ...
      <template v-slot:footer>
        ...
        <HiButton color="primary" size="lg" @click="handleEditGroup(editedGroup)">수정</HiButton>
      </template>
    </HiModal>
    ...
    <HiModal type="type01" size="xs" v-if="modals.groupDel" closeSkip @close="closeModal('groupDel')">
      ...
      <template v-slot:footer>
        ...
        <HiButton color="info" size="lg" @click="handleDeleteGroup(group)">삭제</HiButton>
      </template>
    </HiModal>
...
export default {
  ...
  methods: {
    //모달 열기
    openModal(modalName, group = null) {
    ...
    //그룹명 수정
    handleEditGroup() {
      alert(this.editedGroup.value);
      const index = this.groups.findIndex((g) => g.value === this.editedGroup.value);
      alert(index);
      if (index !== -1) {
        this.$set(this.groups, index, this.editedGroup); // 해당 그룹을 업데이트
        this.closeModal("groupEdit"); // 모달 닫기
      }
    },
